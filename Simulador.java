...
// Clase main. Arma y corre la simulacion.
// Hay 1 hilo Generador, 1 Despachador y N Interceptores (recurso compartido, controlado con un Semaphore(N)).
...
public class Simulador {
    ...
    // menu para elegir escenario, N, recarga y estrategia
    private static void menu() {
        Scanner sc = new Scanner(System.in);

        System.out.println("Sistema concurrente de intercepcion de amenazas aereas");
        System.out.println("(toca Enter para usar el valor entre parentesis)");
        System.out.println();
    ...
    // pasa la opcion 1-4 del menu al nombre de archivo
    ...
    // lee un entero de consola, si esta vacio o esta mal escrito usa porDefecto
    ...
    private static void ejecutar(String archivo, int n, int recarga, Estrategia estrategia) {
        ...
        System.out.println("Interceptores: " + n);
    ...
    // corre una simulacion completa y devuelve el resultado
    private static Resultado correr(String archivo, int n, int recarga, Estrategia estrategia, boolean verbose) {
        ...
        Despachador despachador = new Despachador(cola, stats, reloj, recarga, estrategia, interceptores, recursos, verbose);
    ...
    // corre las 3 estrategias con varias recargas y muestra una tabla comparativa
    private static void batch(String archivo) {
        int[] recargas = {500, 1000, 1500};
        int[] estrategias = {1, 2, 3};
        int n = CANT_INTERCEPTORES;

        System.out.println("COMPARACION DE ESTRATEGIAS (archivo=" + archivo + ", N=" + n + ")");
        System.out.println("Estrategia | recarga | gen | inter | impac | espera | tasa% | critImp | util%");
        ...
        for (int eid : estrategias) {
            for (int recarga : recargas) {
                ...
                System.out.println(est.nombre() + " | " + recarga + " | " + gen + " | " + inter + " | " + impac + " | " + r.stats.getEsperaPromedio() + " | "
                        + Math.round(tasa) + "% | " + r.stats.getCriticidadImpactada() + " | " + Math.round(util) + "%");
            }
            System.out.println();
        }
